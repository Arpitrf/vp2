...
def compute_fvd(cfg):
    ...
    dataset_file = "/home/arpit/test_projects/OmniGibson/dynamics_model_dataset_seg/dataset.hdf5"
    dataset_name = 'og'
    ...
    for i, batch in enumerate(test_data_loader):
        ...
        batch['actions'] = torch.tensor([[[-2.7076e-02, -9.7618e-03,  4.3813e-03,  4.4717e-02,  1.9045e-02,
         -3.6109e-02,  1.0000e+00],
        [-1.9943e-02, -6.2497e-03, -4.7518e-02,  4.6406e-02,  1.6447e-02,
         -4.4320e-02,  1.0000e+00],
        [-1.1995e-02, -2.0442e-02, -3.6335e-02,  4.4674e-02,  1.3784e-02,
         -4.4399e-02,  1.0000e+00],
        [-1.4401e-02,  2.7766e-03, -5.0049e-02,  4.6386e-02,  1.0184e-02,
         -4.1680e-02,  1.0000e+00],
        [-1.4928e-03, -1.5766e-02, -4.6233e-02,  4.9908e-02,  1.3830e-02,
         -3.6026e-02,  1.0000e+00],
        [ 2.6211e-02, -1.4577e-02, -3.1292e-02,  4.9647e-02,  1.4000e-02,
         -3.8782e-02,  1.0000e+00],
        [ 6.1351e-03,  1.3970e-02, -5.1359e-02,  3.7412e-03,  3.0398e-03,
          4.0468e-04,  1.0000e+00]]])
        
        
        for k in batch.keys():
            # print("k: ", k)
            if batch[k] is not None:
                # print("batch[k]: ", batch[k].shape, batch[k].dtype)
        ...
        print("eval_preds: ", eval_preds['gripper_obj_seg'].shape)
        # remove later
        # pred_video = eval_preds[ag_mode]["rgb"].permute(0, 1, 3, 4, 2)
        pred_video = eval_preds['gripper_obj_seg']
        pred_torch = torch.from_numpy(pred_video)
        logSoftmax = torch.nn.LogSoftmax(dim=-1)
        out = logSoftmax(pred_torch).cpu().numpy()
        pred_seg_img = np.argmax(out, axis=-1)
        fig, ax = plt.subplots(1,8)
        for i in range(8):
            ax[i].imshow(pred_seg_img[0,i])
        plt.show()
